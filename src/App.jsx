...
import { C, F, FM, FN, R, S, H, SH, card, inp, btn, tag, applyTheme, TY, injectKeyframes } from "./theme.js";
...
function App() {
  const [darkMode, setDarkMode] = useState(() => {
    try { return localStorage.getItem("rp26_dark") !== "false"; } catch { return true; }
  });
  ...
  const alertThemes = [];
  const TAB_ICONS = { agenda: "\u{1F4C5}", dashboard: "\u{1F4CA}", revisoes: "\u{1F504}", provas: "\u{1F4DD}", temas: "\u{1F4DA}" };
  const TABS = [
  ...
  const contentStyle = {
    ...
    transform: tabTransition ? "translateY(6px)" : "translateY(0)",
    transition: "opacity 0.2s ease, transform 0.2s ease",
  };

  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: F, color: C.text }}>
      {/* HEADER — desktop/tablet top nav */}
      <div style={{
        paddingTop: "max(env(safe-area-inset-top, 0px), 20px)",
        background: `linear-gradient(160deg, rgba(129,140,248,0.12) 0%, rgba(196,181,253,0.10) 50%, transparent 100%)`,
        ...
        <div style={{
          ...
          {/* Desktop tab bar with animated indicator */}
              ...
              return (
                <button key={t.id} onClick={() => switchTab(t.id)} style={{
                  ...
                  background: active
                    ...
                    : "transparent",
                  border: active ? `1px solid rgba(196,181,253,0.35)` : "1px solid transparent",
                  borderRadius: R.pill,
                  cursor: "pointer",
                  whiteSpace: "nowrap",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0, marginLeft: 8 }}>
            {flash && (
              <span className="fade-slide-in" style={{
          ...
          {TABS.map((t) => {
            ...
            return (
              <button key={t.id} onClick={() => switchTab(t.id)} style={{
                ...
                }}>{t.id === "revisoes" ? "Revis\u00f5es" : t.label}</span>
                {hasNotif && (
                  <span style={{
      ...
      {/* Inject responsive CSS for bottom nav */}
      <style>{`
