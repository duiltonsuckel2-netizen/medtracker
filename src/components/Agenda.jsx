              ...
              <div key={day.id} style={{ background: C.card, border: `1px solid ${isToday ? C.blue + "50" : C.border}`, borderRadius: R.xl, overflow: "hidden", boxShadow: isToday ? SH.glow(C.blue) : SH.sm }}>
                ...
                <div style={{ padding: `${S.lg}px ${S.xl}px ${S.xl}px`, display: "flex", flexDirection: "column", gap: S.sm }}>
                  {day.items.map((item) => {
                    ...
                    return (
                      <div key={item.id} style={{ display: "flex", alignItems: "center", gap: S.md, padding: "12px 14px", background: item.done ? C.card : C.card2, borderRadius: R.md, border: `1px solid ${item.done ? C.green + "15" : C.border}`, transition: "all 0.15s" }}>
                        <div onClick={() => toggleDone(day.id, item.id)} style={{ width: 22, height: 22, borderRadius: 7, border: item.done ? `2px solid ${C.green}` : `2px solid ${C.border2}`, background: item.done ? C.green : "transparent", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0, transition: "all 0.15s" }}>
                        ...
                        {isEd ? <input autoFocus value={editText} onChange={(e) => setEditText(e.target.value)} onBlur={commitEdit} onKeyDown={(e) => { if (e.key === "Enter") commitEdit(); if (e.key === "Escape") setEditing(null); }} style={{ ...inp(), flex: 1, padding: "4px 8px", fontSize: 13, background: "none", border: "none", borderBottom: `1px solid ${C.border2}`, borderRadius: 0 }} />
                          : <span style={{ flex: 1, fontSize: 13, lineHeight: 1.4, color: item.done ? C.text3 : C.text, textDecoration: item.done ? "line-through" : "none", opacity: item.done ? 0.5 : 1 }}>{item.text}</span>}
