  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      {step === 0 && <>
        ...
        {exams.length === 0 ? <Empty msg="Nenhuma prova registrada ainda." /> : exams.map((exam) => <ExamCard key={exam.id} exam={exam} allLogs={allLogs} isOpen={detail === exam.id} onToggle={() => setDetail(detail === exam.id ? null : exam.id)} onDel={onDel} onUpdate={onUpdate} knownThemes={knownThemes} />)}
      </>}
  ...
  return (
    <div style={card}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }} onClick={onToggle}>
        <div style={{ width: 68, height: 68, borderRadius: R.lg, background: perfColor(geral) + "15", border: `2px solid ${perfColor(geral)}40`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flexShrink: 0, boxShadow: SH.glow(perfColor(geral)) }}><span style={{ fontSize: 20, fontWeight: 700, color: perfColor(geral), ...NUM, lineHeight: 1 }}>{geral}%</span><span style={{ fontSize: 9, color: C.text3, ...NUM }}>{acertos}/{total}</span></div>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            {CATS.map((cat) => { const cnt = cats[cat.id]?.length || 0; if (!cnt) return null; return (<div key={cat.id} style={{ display: "flex", alignItems: "center", gap: 5 }}><div style={{ width: 9, height: 9, borderRadius: "50%", background: cat.color, flexShrink: 0 }} /><span style={{ fontSize: 11, color: cat.color, fontWeight: 600 }}>{cnt}</span></div>); })}
          </div>
        </div>
      ...
      {isOpen && <div style={{ marginTop: 16, paddingTop: 16, borderTop: `1px solid ${C.border}`, display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:S.md}}>{CATS.map((cat)=>{const cnt=cats[cat.id]?.length||0;const p=total>0?perc(cnt,total):0;return(<div key={cat.id} style={{background:C.surface,borderRadius:R.md,padding:S.md,border:`1px solid ${cat.color}30`,textAlign:"center",boxShadow:SH.sm}}><div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6,marginBottom:4}}><div style={{width:10,height:10,borderRadius:"50%",background:cat.color,flexShrink:0}}/><span style={{fontSize:22,fontWeight:800,color:cat.color,...NUM}}>{cnt}</span></div><div style={{fontSize:10,color:cat.color,fontFamily:FM,marginBottom:2}}>{p}%</div><div style={{fontSize:10,color:C.text3,lineHeight:1.3}}>{cat.label}</div></div>);})}</div>
        <div><div style={{fontSize:13,fontWeight:600,marginBottom:10}}>Por área</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(175px,1fr))",gap:S.md}}>{AREAS.map((a)=>{const st=areaStats[a.id];const t2=st.total||24;const ac2=st.soube+st.chutou;const p2=perc(ac2,t2);const diff=p2-BENCHMARKS[a.id];return(<div key={a.id} style={{background:C.surface,borderRadius:R.md,padding:S.lg,border:`1px solid ${a.color}25`,boxShadow:SH.sm}}><div style={{fontSize:12,fontWeight:600,color:a.color,marginBottom:6}}>{a.label}</div><div style={{fontSize:20,fontWeight:700,color:perfColor(p2),...NUM}}>{p2}%</div><div style={{height:4,background:C.card,borderRadius:R.pill,overflow:"hidden",margin:"6px 0"}}><div style={{height:"100%",width:`${p2}%`,background:perfColor(p2),borderRadius:999}}/></div><div style={{fontSize:10,color:C.text3,fontFamily:FM}}>meta 85% <span style={{color:diff>=0?"#34D399":"#F87171"}}>{diff>=0?`+${diff}pp`:`${diff}pp`}</span></div><div style={{fontSize:10,color:C.text3,fontFamily:FM,marginTop:4,display:"flex",gap:6,flexWrap:"wrap"}}>{CATS.map((cat)=><span key={cat.id} style={{color:cat.color}}>{cat.short}:{st[cat.id]||0}</span>)}</div></div>);})}</div></div>
        {ev>0&&(<div style={{background:C.surface,border:`1px solid ${CATS[2].color}30`,borderRadius:R.lg,padding:S.lg}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}><div style={{width:10,height:10,borderRadius:"50%",background:CATS[2].color,flexShrink:0}}/><span style={{fontSize:13,fontWeight:600,color:CATS[2].color}}>Errei mas já estudei — revisar com urgência ({ev})</span></div><div style={{display:"flex",flexDirection:"column",gap:10}}>{AREAS.map((a)=>{const qs=evByArea[a.id];if(!qs.length)return null;return(<div key={a.id}><div style={{fontSize:11,fontWeight:600,color:a.color,marginBottom:6,textTransform:"uppercase",letterSpacing:0.5}}>{a.label}</div><div style={{display:"flex",flexDirection:"column",gap:4}}>{qs.map((q)=>(<div key={q.n} style={{display:"flex",gap:8,alignItems:"center",padding:"6px 10px",background:CATS[2].color+"0D",borderRadius:8}}><span style={{fontSize:10,fontWeight:700,color:CATS[2].color,fontFamily:FM,minWidth:24}}>Q{q.n}</span><div style={{width:7,height:7,borderRadius:"50%",background:CATS[2].color,flexShrink:0}}/><span style={{fontSize:12,flex:1}}>{q.theme}</span>{q.schedule&&<span style={{fontSize:10,color:C.blue,fontFamily:FM,background:C.blue+"14",padding:"1px 6px",borderRadius:4,flexShrink:0}}>📅 {q.schedule.semana}</span>}</div>))}</div></div>);})}</div></div>)}
        {en>0&&(<div style={{background:C.surface,border:`1px solid ${CATS[3].color}30`,borderRadius:R.lg,padding:S.lg}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><div style={{width:10,height:10,borderRadius:"50%",background:CATS[3].color,flexShrink:0}}/><span style={{fontSize:13,fontWeight:600,color:CATS[3].color}}>Nunca estudei — {en} gaps a cobrir</span></div><div style={{fontSize:11,color:C.text3,fontFamily:FM,marginBottom:12}}>Prevalência: Unicamp · USP-SP · UNIFESP · SUS-SP · IAMSPE · ISCMSP</div><div style={{display:"flex",flexDirection:"column",gap:14}}>{AREAS.map((a)=>{const qs=enByArea[a.id];if(!qs.length)return null;const sorted=[...qs.map((q)=>({...q,p:NMAP[q.n]||"baixa"}))].sort((x,y)=>(PORD[x.p]??3)-(PORD[y.p]??3));return(<div key={a.id}><div style={{fontSize:11,fontWeight:600,color:a.color,marginBottom:6,textTransform:"uppercase",letterSpacing:0.5}}>{a.label}</div><div style={{display:"flex",flexDirection:"column",gap:3}}>{sorted.map((q)=>{const st2=PSTYLE[q.p]||PSTYLE["baixa"];return(<div key={q.n} style={{display:"flex",gap:8,alignItems:"center",padding:"5px 10px",background:st2.color+"10",borderRadius:8}}><div style={{width:7,height:7,borderRadius:"50%",background:CATS[3].color,flexShrink:0}}/><span style={{fontSize:10,fontWeight:700,color:st2.color,fontFamily:FM,minWidth:64,flexShrink:0}}>{st2.label}</span><span style={{fontSize:12,flex:1}}>{q.theme}</span>{q.schedule&&<span style={{fontSize:10,color:C.blue,fontFamily:FM,background:C.blue+"14",padding:"1px 6px",borderRadius:4,flexShrink:0}}>📅 {q.schedule.semana}</span>}</div>);})}</div></div>);})}</div></div>)}
      </div>}
    </div>
  );
}
