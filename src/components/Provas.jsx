...
import { AREAS, BENCHMARKS, CATS, areaMap, EXAM_THEMES_DB, KNOWN_PDFS, UFCSPA_2026_ANALYSIS } from "../data.js";
...
function ExamCard({ exam, allLogs, isOpen, onToggle, onDel, onUpdate, knownThemes }) {
  ...
  const isUfcspa2026 = (exam.name || "").toLowerCase().includes("ufcspa") && (exam.name || "").includes("2026");
  const analysis = isUfcspa2026 ? UFCSPA_2026_ANALYSIS : null;
  return (
    <div style={card}>
      ...
      {isOpen && <div style={{ marginTop: 16, paddingTop: 16, borderTop: `1px solid ${C.border}`, display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {analysis&&<div style={{background:C.surface,border:`1px solid ${C.purple}30`,borderRadius:R.lg,padding:S.lg}}>
          ...
          <div style={{fontSize:11,fontWeight:600,color:C.green,marginBottom:6,textTransform:"uppercase",letterSpacing:0.5}}>Temas que SEMPRE caem na UFCSPA (4/4 provas)</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:4}}>{analysis.sempreCAI.map((s,i)=><div key={i} style={{fontSize:10,padding:"4px 10px",borderRadius:R.pill,background:C.green+"14",border:`1px solid ${C.green}25`,color:C.green,fontWeight:600}}>Q{s.q} {s.tema}</div>)}</div>
        </div>}
      </div>}
    </div>
  );
