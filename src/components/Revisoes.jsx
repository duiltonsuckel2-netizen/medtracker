            ...
            <div key={r.id} style={{ ...card, borderLeft: `3px solid ${C.red}`, marginBottom: S.sm, boxShadow: SH.sm }}>
              <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                <div style={{ flex: 1 }}>
                  ...
                  {(() => { const subRevs = reviews.filter(sr => sr.isSubtopic && sr.parentTheme && r.theme && sr.area === r.area && sr.parentTheme.toLowerCase().trim() === r.theme.toLowerCase().trim()); const stNames = getSubtopicsForReview(r); const allSubs = subRevs.length > 0 ? subRevs : stNames.map((n, i) => ({ id: `st${i}`, theme: n })); if (allSubs.length === 0 && stNames.length > 0) return ( <div style={{ marginTop: 6, display: "flex", gap: 4, flexWrap: "wrap" }}>{stNames.map((n, i) => <span key={i} style={{ fontSize: 10, padding: "2px 8px", borderRadius: R.pill, background: C.surface, border: `1px solid ${C.border}`, fontFamily: FM, lineHeight: 1.4 }}>{n}</span>)}</div> ); return allSubs.length > 0 ? (
                    <div style={{ marginTop: 6, display: "flex", gap: 4, flexWrap: "wrap" }}>
                      {allSubs.map(s => <span key={s.id} style={{ fontSize: 10, padding: "2px 8px", borderRadius: R.pill, background: s.lastPerf != null ? perfColor(s.lastPerf) + "14" : C.surface, border: `1px solid ${s.lastPerf != null ? perfColor(s.lastPerf) + "30" : C.border}`, fontFamily: FM, lineHeight: 1.4 }}>{s.theme}{s.lastPerf != null && <span style={{ fontWeight: 700, color: perfColor(s.lastPerf), marginLeft: 3 }}>{s.lastPerf}%</span>}</span>)}
                    </div>
                  ) : null; })()}
                  {r.subtemaNote && (
                ...
                return (
                  <div key={r.id} style={{ ...card, padding: `${S.lg}px`, borderRadius: R.lg, borderLeft: `3px solid ${aColor}`, display: "flex", alignItems: "center", gap: S.lg }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {(() => { const subRevs = reviews.filter(sr => sr.isSubtopic && sr.parentTheme && r.theme && sr.area === r.area && sr.parentTheme.toLowerCase().trim() === r.theme.toLowerCase().trim()); const stNames = getSubtopicsForReview(r); const allSubs = subRevs.length > 0 ? subRevs : stNames.map((n, i) => ({ id: `st${i}`, theme: n })); return allSubs.length > 0 ? (
