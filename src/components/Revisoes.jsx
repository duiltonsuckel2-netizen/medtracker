                ...
                return (
                  <div key={r.id} style={{ ...card, padding: `${S.lg}px`, borderRadius: R.lg, borderLeft: `3px solid ${aColor}`, display: "flex", alignItems: "center", gap: S.lg }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {(() => { const allSubs = resolveSubtopicsWithScores(r); const urgentSubs = allSubs.filter(s => s.isDue || (s.days != null && s.days <= 7)); const laterSubs = allSubs.filter(s => !s.isDue && (s.days == null || s.days > 7)); return allSubs.length > 0 ? (
                        <div style={{ marginTop: 4, display: "flex", flexDirection: "column", gap: 3 }}>
                          {urgentSubs.length > 0 && <div style={{ display: "flex", gap: 3, flexWrap: "wrap", alignItems: "center" }}>
                            <span style={{ fontSize: 8, color: C.red, fontWeight: 600, fontFamily: FM }}>REVISAR:</span>
                            {urgentSubs.map((s, i) => <span key={i} style={{ fontSize: 9, padding: "1px 6px", borderRadius: R.pill, background: C.red + "14", border: `1px solid ${C.red}35`, fontFamily: FM, lineHeight: 1.4, fontWeight: 600 }}>{s.name}{s.pct != null && <span style={{ fontWeight: 700, color: perfColor(s.pct), marginLeft: 2 }}>{s.pct}%</span>}{s.intervalIndex != null && <span style={{ color: C.text3, fontWeight: 500, marginLeft: 2 }}>{INT_LABELS[s.intervalIndex]}</span>}{s.days != null && s.days > 0 && <span style={{ color: C.text3, fontWeight: 500, marginLeft: 2 }}>em {s.days}d</span>}</span>)}
                          </div>}
                          {laterSubs.length > 0 && <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                            {laterSubs.map((s, i) => <span key={i} style={{ fontSize: 9, padding: "1px 6px", borderRadius: R.pill, background: s.pct != null ? perfColor(s.pct) + "14" : C.surface, border: `1px solid ${s.pct != null ? perfColor(s.pct) + "30" : C.border}`, fontFamily: FM, lineHeight: 1.4, opacity: 0.7 }}>{s.name}{s.pct != null && <span style={{ fontWeight: 700, color: perfColor(s.pct), marginLeft: 2 }}>{s.pct}%</span>}{s.intervalIndex != null && <span style={{ color: C.text3, fontWeight: 500, marginLeft: 2 }}>{INT_LABELS[s.intervalIndex]}</span>}{s.days != null && <span style={{ color: C.text3, fontWeight: 500, marginLeft: 2 }}>em {s.days}d</span>}</span>)}
                          </div>}
                        </div>
                      ) : null; })()}
                    </div>
