        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 8 }}>
            {AREAS.map((a) => { const active = form.area === a.id; return (
              <button key={a.id} onClick={() => { set("area", a.id); set("theme", ""); }} style={{ padding: "14px 6px", background: active ? a.color + "12" : C.surface, border: active ? `1px solid ${a.color}40` : `1px solid ${C.border}`, borderRadius: R.xl, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 6, transition: "all 0.15s", boxShadow: active ? SH.md : SH.sm, minHeight: 80, opacity: active ? 1 : 0.75 }}>
