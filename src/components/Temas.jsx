...
import { AREAS, INTERVALS, INT_LABELS, areaMap, SEMANAS, AREA_SHORT_MAP } from "../data.js";
...
/* Extract week number from theme name like "HAS (Sem. 08)" → 8 */
...
function Temas({ reviews, subtopics, onEditInterval, onSaveSubtopics }) {
  ...
  // Group filtered reviews by week (ordered Sem. 01 → 02 → …, then "Sem avulsos" at end)
  const weekGroups = useMemo(() => {
    const byWeek = new Map();
    filtered.forEach((r) => {
      const wk = extractWeek(r.theme);
      const key = wk !== null ? wk : Infinity;
    ...
    return sorted.map(([wk, items]) => {
      const semLabel = wk === Infinity ? null : `Sem. ${String(wk).padStart(2, "0")}`;
      const semData = semLabel ? SEMANAS.find((s) => s.semana === semLabel) : null;
      return { week: wk, label: semLabel || "Sem semana", semData, items };
    });
  }, [filtered]);

  // Flat list for pagination
  const allItems = useMemo(() => weekGroups.flatMap((g) => g.items), [weekGroups]);
  const visibleIds = useMemo(() => new Set(allItems.slice(0, visibleCount).map((r) => r.id)), [allItems, visibleCount]);
  ...
  function getSubtopicsForTheme(r) {
    ...
    // 1) Try direct key match first (most reliable)
    const directKey = `${r.area}__${r.theme}`;
    if (subtopics[directKey] && subtopics[directKey].length > 0) {
      return { key: directKey, items: subtopics[directKey] };
    }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ fontSize: 12, color: C.text3, fontFamily: FM }}>
        {filtered.length} temas{stCount > 0 ? ` · ${stCount} subtemas rastreados` : ""} · ordenado pelo cronograma
      </div>
      {filtered.length === 0
        ? <Empty icon={"🔍"} msg={search ? `Nenhum tema para "${search}"` : "Nenhum tema encontrado."} />
        : weekGroups.map((group) => {
          const visibleItems = group.items.filter((r) => visibleIds.has(r.id));
          if (visibleItems.length === 0) return null;
          const isWeekCollapsed = expandedWeeks[group.week] === false;
          const weekDue = group.items.some((r) => r.nextDue <= today());
          const weekTopics = group.semData ? group.semData.aulas.map((a) => `${a.area}: ${a.topic}`).join(" · ") : null;

          return (
            <div key={group.week} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {/* Week header */}
              <div
                onClick={() => toggleWeek(group.week)}
                style={{
                  display: "flex", alignItems: "center", gap: 10, cursor: "pointer",
                  padding: "8px 14px", borderRadius: R.lg,
                  background: weekDue ? C.red + "08" : C.surface,
                  border: `1px solid ${weekDue ? C.red + "30" : C.border}`,
                  userSelect: "none",
                }}
              >
                <span style={{ fontSize: 13, fontWeight: 700, color: weekDue ? C.red : C.purple, fontFamily: FM, minWidth: 60 }}>
                  {group.label}
                </span>
                {weekTopics && (
                ...
                <span style={{ fontSize: 11, color: C.text3, fontFamily: FM }}>{group.items.length} tema{group.items.length > 1 ? "s" : ""}</span>
                {weekDue && <span style={{ fontSize: 9, fontWeight: 600, color: C.red, padding: "2px 6px", background: C.red + "12", borderRadius: R.pill }}>vencidas</span>}
                <span style={{ fontSize: 10, color: C.text3 }}>{isWeekCollapsed ? "▶" : "▼"}</span>
              </div>
              {/* Week items */}
              {!isWeekCollapsed && visibleItems.map((r) => {
                ...
                return (
                  <div key={r.id} className="fade-in" style={{ borderRadius: R.xl, overflow: "hidden", boxShadow: isDue ? SH.glow(C.red) : SH.md, marginLeft: 12 }}>
      ...
      {hasMore && <button onClick={() => setVisibleCount((v) => v + PAGE_SIZE)} style={btn(C.surface, { color: C.text2, border: `1px solid ${C.border}`, width: "100%", textAlign: "center" })}>Carregar mais ({allItems.length - visibleCount} restantes)</button>}
    </div>
  );
}
export { Temas };
